...
public partial class LootWindow
{
    ...
    private void CustomLootTab()
    {
        ...
        var limit = useLimit ? Plugin.Configuration.DateLimit != DateLimit.None ? $"超過 {Plugin.Configuration.DateLimit.GetName()}" : $"從 {CustomMinDate.ToLongDateWithoutWeekday()} 到 {CustomMaxDate.ToLongDateWithoutWeekday()}" : "";
